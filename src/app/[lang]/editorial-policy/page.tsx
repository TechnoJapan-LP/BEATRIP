import type { Metadata } from "next";
import Link from "next/link";
import { Header } from "@/components/header";
import { Breadcrumbs } from "@/components/breadcrumbs";
import { SiteFooter } from "@/components/site-footer";
import { getDictionary, hasLocale } from "../dictionaries";
import { localizeHref } from "@/lib/i18n/locale";
import { CONTACT_EMAIL } from "@/lib/site-config";

// 静的コンテンツのため 1 日ごとの再検証で十分
export const revalidate = 86400;

export async function generateMetadata({
  params,
}: {
  params: Promise<{ lang: string }>;
}): Promise<Metadata> {
  const { lang } = await params;
  const locale = hasLocale(lang) ? lang : "ja";
  const dict = await getDictionary(locale);
  return {
    title: dict.editorialPolicy.metaTitle,
    description: dict.editorialPolicy.metaDescription,
    alternates: {
      canonical:
        locale === "en"
          ? "https://beatrip.jp/en/editorial-policy"
          : "https://beatrip.jp/editorial-policy",
      languages: {
        ja: "https://beatrip.jp/editorial-policy",
        en: "https://beatrip.jp/en/editorial-policy",
        "x-default": "https://beatrip.jp/editorial-policy",
      },
    },
  };
}

export default async function EditorialPolicyPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  const { lang } = await params;
  const locale = hasLocale(lang) ? lang : "ja";
  const dict = await getDictionary(locale);
  const t = dict.editorialPolicy;

  return (
    <>
      <Header />
      <main
        id="main-content"
        className="mx-auto w-full max-w-4xl flex-1 px-4 py-8 sm:px-6"
      >
        <div className="mb-6">
          <Breadcrumbs
            currentPath={localizeHref("/editorial-policy", locale)}
            items={[
              { label: dict.common.home, href: localizeHref("/", locale) },
              { label: t.breadcrumb },
            ]}
          />
        </div>

        <h1 className="font-heading text-3xl tracking-wide text-zinc-900 uppercase dark:text-zinc-100 sm:text-4xl">
          {t.title}
        </h1>
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
          {t.updated}
        </p>
        <p className="mt-4 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400 max-w-2xl">
          {t.intro}
        </p>

        <div className="mt-8 space-y-6">
          {t.sections.map((section) => (
            <div
              key={section.title}
              className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6"
            >
              <h2 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-3">
                {section.title}
              </h2>
              <p className="text-xs leading-relaxed text-zinc-600 dark:text-zinc-400 whitespace-pre-line">
                {section.content}
              </p>
            </div>
          ))}

          {/* 訂正・お問い合わせ窓口 (site-config の CONTACT_EMAIL を一元参照) */}
          <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6">
            <h2 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-3">
              {t.contactTitle}
            </h2>
            <p className="text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
              {t.contactBody}
            </p>
            <p className="mt-3 text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
              {CONTACT_EMAIL ? (
                <a
                  href={`mailto:${CONTACT_EMAIL}`}
                  className="font-bold underline underline-offset-2 text-zinc-900 dark:text-zinc-100"
                >
                  {CONTACT_EMAIL}
                </a>
              ) : (
                t.contactPending
              )}
            </p>
          </div>
        </div>

        {/* 関連ページ (/disclosure・/about と相互リンク) */}
        <section className="mt-10 mb-8">
          <h2 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-3">
            {t.relatedTitle}
          </h2>
          <ul className="space-y-2">
            <li>
              <Link
                href={localizeHref("/disclosure", locale)}
                className="text-sm text-zinc-600 underline underline-offset-2 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100"
              >
                {t.relatedDisclosure}
              </Link>
            </li>
            <li>
              <Link
                href={localizeHref("/about", locale)}
                className="text-sm text-zinc-600 underline underline-offset-2 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100"
              >
                {t.relatedAbout}
              </Link>
            </li>
          </ul>
        </section>
      </main>
      <SiteFooter lang={lang} />
    </>
  );
}
