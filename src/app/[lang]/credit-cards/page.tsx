...
type CardEntry = {
  ...
  pitch: string;
  /**
   * asp-partners.ts の id。env (ASP click URL / a8mat) 設定済みのときだけ
   * 「公式サイトで申込」CTA を出す (未設定なら情報カードのまま)。
   */
  partnerId?: string;
};
      ...
      <main id="main-content" className="mx-auto w-full max-w-7xl flex-1 px-4 py-10 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
            ...
            {SEGMENTS.map((seg) => (
              <section key={seg.id} id={seg.id}>
                ...
                <div className="space-y-3">
                  {seg.cards.map((card) => {
                    const partner = card.partnerId
                      ? getAspPartner(card.partnerId)
                      : undefined;
          ...
          <aside className="space-y-6 lg:sticky lg:top-20 lg:self-start">
            <TableOfContents
              items={[
                ...SEGMENTS.map((s) => ({ id: s.id, label: s.title })),
                { id: "faq", label: "よくある質問" },
              ]}
