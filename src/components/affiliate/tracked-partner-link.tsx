...
export type TrackedPartnerAccent = keyof typeof ACCENT_CLASS;

/**
 * partner クリック計測専用の最小クライアント wrapper。
 * 親 (JapanesePartnersPanel) は Server Component のままにし、
 * onClick が必要な <a> のみここで囲うことでクライアント bundle を最小化する。
 *
 * asp-partners.ts (env 依存) には依存させない。必要な引数だけ受け取る。
 */
export function TrackedPartnerLink({
  href,
  partnerId,
  category,
  destinationCode,
