...
/**
 * 検出しきい値: 前回比 30% 以上の下落 + 絶対額 ¥3,000 以上 (ノイズ除去)。
 *
 * 2026-07-18 に 25% → 30% へ引き上げた。「超お買い得」を名乗る以上、
 * よくある 25% 引きが混ざると特別さが薄れるため。変更時点の掲載中
 * (active) では 10件中6件が 30% 以上で、体感で4割ほど減る想定。
 *
 * **この値は「記録するかどうか」も決めている点に注意。** 下回った下落は
 * KV に残らず、X/Bluesky への速報も飛ばない。つまり 25〜29% の下落は
 * 今後いっさい観測されず、後から「実はその帯が一番売れていた」という
 * 分析はできなくなる。運営者了承済み (2026-07-18)。戻すならこの定数を
 * 下げるだけでよいが、下げても遡って記録は復活しない。
 */
